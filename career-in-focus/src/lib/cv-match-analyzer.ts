...
const SYSTEM_PROMPT = `אתה יועץ קריירה בכיר ב"קריירה בפוקוס" עם 15+ שנות ניסיון בגיוס בישראל. אתה מנתח בקליק קורות חיים מול תיאור תפקיד ספציפי, ומחזיר ניתוח התאמה מקצועי, ישיר, ושימושי.
...
נתח את ההתאמה. החזר JSON תקני בלבד, ללא markdown fences, ללא הקדמה.`;
}
...
export async function analyzeCvMatch(
  ...
  if (!jobText || jobText.trim().length < 30) {
    throw new CvMatchError("bad-input", "תיאור המשרה קצר מדי לניתוח");
  }
