...
export function renderOldLeadsContent(body: string): string {
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith("•") || line.startsWith("-")) {
      ...
      // Feature bullet shape: `**Title** : description` (Coral asked
      // for no em-dashes anywhere). Old templates with `—` and `–` are
      // still accepted as separators so legacy content keeps rendering.
