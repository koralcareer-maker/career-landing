...
import { importRachelliData, type ImportResult } from "@/lib/actions/admin-imports";
...
export function ImportTrigger() {
  ...
  async function run() {
    ...
    setResult(null);
    try {
      const r = await importRachelliData();
      setResult(r);
    } catch (e) {
      setResult({
        ok: false,
          ...
          <div className="flex-1">
            ...
            {result.details && (
              <ul className="mt-2 text-xs space-y-0.5 opacity-90">
                <li>✓ פרופיל עודכן</li>
                <li>✓ תמונת פרופיל הוגדרה</li>
                <li>✓ קורות חיים הוגדרו</li>
